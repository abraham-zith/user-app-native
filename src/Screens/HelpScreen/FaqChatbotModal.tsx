...
const CATEGORY_OPTIONS = [
    ...
    { key: 'lost_item', label: '🔍 Lost Item', icon: 'search-outline' },
    { key: 'app_crash', label: '🐛 App Issue', icon: 'bug-outline' },
    { key: 'account', label: '👤 Account', icon: 'person-outline' },
    { key: 'driver_issue', label: '🚗 Driver Issue', icon: 'car-outline' },
    { key: 'safety', label: '🛡️ Safety', icon: 'shield-checkmark-outline' },
...
const QUICK_REPLIES = [
    { label: '📍 Where is my driver?', text: 'Where is my driver?' },
    { label: '🔍 Lost an item', text: 'I lost an item in the car' },
    { label: '💰 Payment Issue', text: 'I have a payment issue' },
    { label: '🛡️ Safety', text: 'I have a safety concern' },
...
const FaqChatbotModal = ({ visible, onClose }: any) => {
    ...
    }, [isLiveMode, activeTicketId, user]);
    ...
    const handleAgentTimeout = () => {
        const fallbackMsg: Message = {
            id: 'timeout-' + Date.now(),
            text: "⌛ All our agents are currently busy assisting other users. Your ticket is recorded, and we will notify you when an agent replies. You can close this chat or return to the bot helper.",
            sender: 'bot',
            timestamp: Date.now(),
            action: 'return_ai'
        };
    ...
    const generateBotResponse = (userText: string): { text: string; action?: Message['action'] } => {
        ...
        // Advanced AI Logic Additions for Riders
        if (text.includes('lost') || text.includes('item') || text.includes('forgot') || text.includes('belonging')) {
            return { text: "Oh no! If you left something in the car, you can contact the driver directly from the 'Trip History' page for up to 24 hours. Alternatively, tap below to create a ticket and our support team will help you." };
        }

        if (text.includes('where') || text.includes('driver') || text.includes('arriving') || text.includes('late')) {
            return { text: "Your driver's location is shown live on the map. If the driver is delayed, you can call or message them using the icons on the ride screen." };
        }

        if (text.includes('cancel') || text.includes('fee') || text.includes('charge')) {
            return { text: "Cancellation fees apply if you cancel more than 3 minutes after a driver accepts, or if the driver waits at the pickup location for more than 5 minutes. If you were unfairly charged, please open a ticket." };
        }

        if (text.includes('price') || text.includes('fare') || text.includes('payment') || text.includes('money')) {
            return { text: "Fares are calculated upfront based on distance, time, and current demand. If you experienced an issue with your payment, please let an agent know." };
        }
        ...
        if (nextFail === 1) {
            return { text: "I'm sorry, I didn't quite get that. Could you try rephrasing? I can help with finding a lost item, payment issues, or ride tracking." };
        }
    ...
    const handleCategorySelected = async (categoryKey: string) => {
        ...
        try {
            const res = await createTicket({
                ...
                description: `User requested live support via chatbot. Category: ${categoryKey}`,
                priority: 'high',
    ...
    const handleReturnToAI = () => {
        if (activeTicketId) {
            socket.emit("endSupportChat", { ticketId: activeTicketId, userId: user?.id });
        }
        setIsLiveMode(false);
        setActiveTicketId(null);
        clearTimeoutTimer();
