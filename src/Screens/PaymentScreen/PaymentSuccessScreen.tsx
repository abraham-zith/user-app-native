...
const PaymentSuccessScreen: React.FC = () => {
  ...
  const { targetScreen, tripData } = route.params || {};
  ...
  const handleContinue = () => {
    if (targetScreen) {
      navigation.replace(targetScreen, tripData);
    } else {
      navigation.goBack();
    }
  };
      ...
      <View style={styles.content}>
        <View style={styles.iconWrapper}>
          <View style={[
            ...
            <MaterialCommunityIcons name="wallet-giftcard" size={60} color="#FFFFFF" />
          </View>
          {particles.map((p, i) => (
        ...
        <Animated.View style={[styles.textContainer, animatedContentStyle]}>
          <Text style={[styles.title, { color: appColors.text }]}>Payment Successful</Text>
          <Text style={[styles.description, { color: isDark ? appColors.secondaryText : '#64748B' }]}>
            Thank you! Your payment has been processed successfully. Your ride details are being finalized.
          </Text>
        </Animated.View>
      </View>
